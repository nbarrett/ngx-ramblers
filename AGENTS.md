...
- **Ticket references**: the subject line ends with `(ref #NNN)`, or `(ref #NNN, #MMM)` for multiple tickets. Use an existing issue (Critical Rule 11); do not open a new one as part of committing. Never mention external contacts or partner names in commit messages or GitHub issues - describe the scenario generically.
