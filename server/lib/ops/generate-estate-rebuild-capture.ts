...
function platformFrom(config: EnvironmentsConfig): EstateRebuildPlatformSnapshot {
  return {
    ...
    globalAtlasPublicKey: asText(config.atlas?.publicKey),
    globalAtlasPrivateKey: asText(config.atlas?.privateKey),
...
function platformSafeValue(fieldId: string, platform: EstateRebuildPlatformSnapshot, includeSecrets: boolean): string {
  ...
  const values: Record<string, string> = {
    ...
    globalAtlasPublicKey: platform.globalAtlasPublicKey,
    globalAtlasPrivateKey: secretOrPresence(platform.globalAtlasPrivateKey, includeSecrets),
