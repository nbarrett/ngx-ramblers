  ...
  if (!env.mongo?.cluster || !env.mongo?.db || !env.mongo?.username || !env.mongo?.password) {
    ...
    if (!client) {
      ...
      try {
        ...
        const mailchimpDoc = await db.collection("config").findOne({key: ConfigKey.MAILCHIMP});
        ...
        const mailchimp = mailchimpDoc?.value as {apiKey?: string} | undefined;
        ...
        return {
          ...
          mailchimpApiKey: asText(mailchimp?.apiKey),
...
function safeValueFor(
  ...
  if (site.error && !fieldId.startsWith("consoleAccess.")) {
    ...
    const values: Record<string, string> = {
      ...
      mailchimpApiKey: secretOrPresence(site.mailchimpApiKey, includeSecrets),
...
async function buildWorkbook(
  ...
  [
    ["NGX-Ramblers estate rebuild capture", ""],
    ["Generated", generatedAtUtc],
    ["Issue", "https://github.com/nbarrett/ngx-ramblers/issues/287"],
    ["Rule", includeSecrets
      ? "SECRETS INCLUDED. This pack contains live passwords, API keys and tokens so the estate can be rebuilt. Store only in a password manager. Do not commit to git or email."
      : "Secret values omitted: secrets show as SET or empty. Re-download with Include secrets for full rebuild values."],
    ["Include secrets", includeSecrets ? "yes" : "no"],
    ["Sources", "staging config.environments (infra + platform) and each site Mongo config collection (system, brevo, mailchimp, salesforce, committee)"],
    ["Sheets", "README | Third-party systems | Field catalogue | Sites directory | Site capture | Platform capture"],
    ["Third-party systems", "One row per integrated system (MongoDB Atlas, AWS, Fly, maps, mail, …) with function and what config holds."],
    ["Layers", "runtime = config.environments[] for that site. application = site Mongo system/brevo/mailchimp/salesforce. people = committee contacts."],
    ["Configured?", "present = value found in live config. empty = not set. error = site database probe failed."],
    ["Regenerate", "Platform admin: /admin/platform/environment-management/estate-rebuild-capture  or  cd server && npm run ops:estate-rebuild-capture"]
  ...
  const capture = workbook.addWorksheet("Site capture");
  capture.addRow([
  ...
  const platform = workbook.addWorksheet("Platform capture");
  platform.addRow([
...
function buildMarkdown(
  ...
  const secretLine = includeSecrets
    ? "> **SECRETS INCLUDED.** Live passwords, API keys and tokens are written so the estate can be rebuilt. Store only in a password manager."
    : "> Secret values omitted (SET or empty). Re-download with **Include secrets** for full rebuild values.";
  const lines: string[] = [
    "# NGX-Ramblers estate rebuild capture",
    "",
    ...
    secretLine,
    "",
    "Built from staging `config.environments` and each site Mongo `config` collection (`system`, `brevo`, `mailchimp`, `salesforce`, `committee`).",
    "",
    "## Regenerate",
    "",
    "From the staging platform admin UI: **Platform → Environment management → Estate rebuild capture**.",
    "",
    "```bash",
  ...
  lines.push(
    "",
    "## Platform capture (from staging config.environments)",
    "",
  ...
  lines.push(
    ...
    `- Site capture rows: **${infraList.length * SITE_FIELDS.length}**`,
...
<head>
  ...
  <title>NGX-Ramblers Estate Rebuild Capture - Private</title>
  ...
  <div class="container">
    ...
    <div class="stats-hero">
      ...
      <div class="stat-card">
        ...
        <div class="stat-detail">${rows.length} capture rows</div>
      </div>
      <div class="stat-card">
    ...
    <div class="alert alert-warning">
      <div>
        ...
        <p>
          ...
          ${includeSecrets
    ? "Live passwords, API keys and tokens are written so the estate can be rebuilt. Store only in a password manager; do not commit or email."
    : "Secrets show as SET or empty. Re-download with <strong>Include secrets</strong> for full rebuild values."}
        </p>
      </div>
    </div>
...
export async function generateEstateRebuildArtifacts(
  ...
  const xlsx = await buildWorkbook(generatedAtUtc, infraList, probes, rows, platformRows, includeSecrets);
  debugLog("Building markdown…");
  const markdown = buildMarkdown(generatedAtUtc, infraList, probes, platformRows, includeSecrets);
  debugLog("Building HTML…");
  const html = buildHtml(generatedAtUtc, infraList, probes, rows, platformRows, includeSecrets);
  const suffix = includeSecrets ? "-with-secrets" : "";

  return {
    generatedAtUtc,
    siteCount: infraList.length,
    fieldsPerSite: SITE_FIELDS.length,
    siteCaptureRows: rows.length,
    platformFieldCount: PLATFORM_FIELDS.length,
    includeSecrets,
    xlsx,
    markdown,
    html,
    xlsxFileName: `estate-rebuild-capture${suffix}.xlsx`,
    markdownFileName: `estate-rebuild-capture${suffix}.md`,
    htmlFileName: `estate-rebuild-capture${suffix}.html`
  };
}
...
if (require.main === module) {
  generateEstateRebuildCapture({writeToDisk: true})
    .then(result => {
      ...
      debugLog("  site capture rows: %d", result.siteCaptureRows);
