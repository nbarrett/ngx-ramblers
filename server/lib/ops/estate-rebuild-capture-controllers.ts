...
import { generateEstateRebuildArtifacts } from "./generate-estate-rebuild-capture";
...
export async function downloadEstateRebuildCapture(req: Request, res: Response): Promise<void> {
  try {
    ...
    debugLog("Generating estate rebuild capture format=%s includeSecrets=%s", format, includeSecrets);
    ...
    if (format === EstateRebuildCaptureFormat.MARKDOWN) {
      ...
      res.setHeader("Content-Disposition", `attachment; filename="estate-rebuild-capture${secretSuffix}-${stamp}.md"`);
      res.send(artifacts.markdown);
    } else if (format === EstateRebuildCaptureFormat.HTML) {
      res.setHeader("Content-Type", "text/html; charset=utf-8");
      res.setHeader("Content-Disposition", `attachment; filename="estate-rebuild-capture${secretSuffix}-${stamp}.html"`);
      res.send(artifacts.html);
    } else {
      res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
      res.setHeader("Content-Disposition", `attachment; filename="estate-rebuild-capture${secretSuffix}-${stamp}.xlsx"`);
      res.send(artifacts.xlsx);
    }
  } catch (error) {
    debugLog("Estate rebuild capture failed: %s", error?.stack || error);
    res.status(500).json({error: error?.message || "Failed to generate estate rebuild capture"});
  }
}
...
export async function estateRebuildCaptureSummary(_req: Request, res: Response): Promise<void> {
  try {
    ...
    debugLog("Estate rebuild capture summary failed: %s", error?.stack || error);
    res.status(500).json({error: error?.message || "Failed to load estate rebuild capture summary"});
  }
}
