...
export async function getConsoleAccess(req: Request, res: Response): Promise<void> {
  try {
    ...
    res.status(500).json({error: error?.message || "Failed to load console access"});
  }
}
...
export async function saveConsoleAccess(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (scope === PLATFORM_SCOPE) {
      ...
      debugLog("Saved platform console access (%d services)", keys(consoleAccess).length);
      res.json({
        scope: PLATFORM_SCOPE,
        environment: null,
        consoleAccess,
      ...
      if (index < 0) {
        ...
        debugLog("Saved console access for %s (%d services)", environment, keys(consoleAccess).length);
        res.json({
          scope: environment,
          environment,
          consoleAccess,
    ...
    res.status(500).json({error: error?.message || "Failed to save console access"});
  }
}
