...
const SITE_RUNTIME_AND_APP_FIELDS: EstateRebuildSiteFieldDefinition[] = [
  ...
  {fieldId: "mailchimpApiKey", category: "Email", label: "Mailchimp API key", whereHeld: "config.mailchimp.apiKey", source: S.MAILCHIMP, layer: L.APPLICATION},
...
export const THIRD_PARTY_SYSTEMS: EstateRebuildThirdPartySystem[] = [
  {
    ...
    informationHeld: "Runtime: cluster, database name, app DB user and password. Console: per-environment Atlas website login under consoleAccess (not the app DB user).",
  ...
  },
  {
    ...
    informationHeld: "Runtime IAM keys and bucket. Console: per-environment AWS website login under consoleAccess.",
  ...
  },
  {
    ...
    informationHeld: "Runtime deploy API token and app settings. Console: per-environment fly.io website login under consoleAccess (not the API token).",
  ...
  },
  {
    systemId: "mailchimp",
    name: "Mailchimp",
    function: "Legacy email provider where still configured instead of (or alongside) Brevo.",
    informationHeld: "API key in the site mailchimp config document.",
    configPaths: "config.mailchimp.apiKey ; system.mailDefaults.mailProvider",
    scope: EstateRebuildSystemScope.PER_SITE
  },
  {
