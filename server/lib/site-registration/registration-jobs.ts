...
import { RegistrationEmailType, RegistrationJobStage, RegistrationNavbarPath, RegistrationPage, RegistrationPlan, RegistrationState, RegistrationStep, RegistrationWalkCandidate, StoredSiteRegistration } from "../../../projects/ngx-ramblers/src/app/models/site-registration.model";
...
import { createEnvironmentMongoUser } from "../environment-setup/mongo-database-user";
...
async function provisionRegistration(registration: StoredSiteRegistration): Promise<void> {
  ...
  const mongoUser = await createEnvironmentMongoUser(details.serviceConfigs.mongodb, database, name);
  const request = {
    ...defaults,
...
async function importRegistration(savedRegistration: StoredSiteRegistration): Promise<void> {
  ...
  const migratedPages = photosByYear(withIntroductionPhotos([...sourcePages, ...informationPages], migrated.albums), registration.pages || []);
  const connection = await connectToEnvironmentMongo(context.envConfigData);
  try {
    ...
    if (importedHome?.rows?.length) {
      const keyAreas = registrationKeyAreas(assembled, migratedPages, migrated.albums);
