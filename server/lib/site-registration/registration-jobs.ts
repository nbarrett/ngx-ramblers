...
import { RegistrationEmailType, RegistrationJobStage, RegistrationNavbarPath, RegistrationPage, RegistrationPlan, RegistrationState, RegistrationStep, StoredSiteRegistration } from "../../../projects/ngx-ramblers/src/app/models/site-registration.model";
...
import { earlierImportFilters, importedPagePath, withRowsUnderHeading, pairedImageRows, registrationMigrationConfig, withIntroductionPhotos, withoutEmptyRows, withPageHeading, withSourcePageAlbums } from "./registration-content";
...
import { scrapeRegistrationSite } from "./registration-import-scrape";
...
async function importRegistration(savedRegistration: StoredSiteRegistration): Promise<void> {
  ...
  const migrated = await migrateRegistrationAssets(registration, preparedPages, result.albums, uploadBucket, importProgress);
  ...
  const migratedPages = withIntroductionPhotos([...sourcePages, ...informationPages], migrated.albums);
  const connection = await connectToEnvironmentMongo(context.envConfigData);
  try {
    ...
    const albumRows = migrated.albums.filter(album => !album.sourcePagePath).map(album => album.pageContent?.rows?.[0]).filter(Boolean);
    if (importedHome?.rows?.length || albumRows.length) {
      await connection.db.collection("pageContent").updateOne({path: "#home-content"}, {$set: {rows: [...albumRows, ...(importedHome?.rows || [])]}}, {upsert: true});
    } else {
    ...
    try {
      ...
      if (registration) {
        ...
        try {
          ...
          await registrations().updateOne({id: registration.id, leaseOwner: workerId}, {$set: {state: RegistrationState.FAILED, error: registrationFailureMessage(stage.name, sanitiseRegistrationMessage(error.message))}});
        } finally {
