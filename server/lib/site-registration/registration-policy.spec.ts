...
import { RegistrationSettings, RegistrationStageStatus, RegistrationState, StoredSiteRegistration } from "../../../projects/ngx-ramblers/src/app/models/site-registration.model";
...
const settings: RegistrationSettings = {
  ...
  approvedEmails: [{groupCode: "AB01", recipients: [{email: "committee@example.com"}]}]
};
...
describe("site registration policy", () => {
  ...
  it("matches an area-only approval for the area itself and never for its groups", () => {
    ...
    expect(registrationEmailAllowed(areaSettings, "AB01", "area@example.com")).toBe(false);
    expect(registrationEmailAllowed(settings, "AB", "committee@example.com")).toBe(false);
  });
  ...
  it("shows the return-link holder a generic failure and keeps the detail for the reviewer", () => {
    ...
    expect(shown.stages.map(stage => stage.status)).toContain(RegistrationStageStatus.FAILED);
...
describe("registration settings problems", () => {
  ...
  it("names the group whose approved email list is invalid, whether or not registration is on", () => {
    const withBadEmail = {...complete, enabled: false, approvedEmails: [{groupCode: "AB01", recipients: [{email: "fine@example.com"}]}, {groupCode: "CD02", recipients: [{email: "broken"}]}]};
