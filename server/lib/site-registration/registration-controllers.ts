...
export async function discoverRegistration(req: Request, res: Response): Promise<void> {
  ...
  const discovered = await discoverRegistrationWebsite(website, saved.group?.group_code);
...
async function queueRetry(saved: {id: string; state: RegistrationState; environmentName?: string; reviewedAt?: number; importedAt?: number; reviewNotifiedAt?: number; leaseUntil?: number}): Promise<void> {
  ...
  if (!environmentExists) {
    await registrations().updateOne({id: saved.id, state: saved.state, leaseUntil: {$lte: dateTimeNowAsValue()}}, {$set: {
      ...
      provisionedAt: null, importedAt: null, reviewedAt: null, reviewNotifiedAt: null, invitedAt: null, error: null
    }});
  } else {
    await registrations().updateOne({id: saved.id, state: saved.state, leaseUntil: {$lte: dateTimeNowAsValue()}}, {$set: {
