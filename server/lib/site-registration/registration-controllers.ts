...
import { RegistrationState } from "../../../projects/ngx-ramblers/src/app/models/site-registration.model";
import { Member } from "../../../projects/ngx-ramblers/src/app/models/member.model";
import { publicRegistration, assertRegistrationEditable, registrationSettingsProblem } from "./registration-policy";
import { registrations, registrationSettings, saveRegistrationSettings } from "./registration-store";
import { startRegistration, confirmRegistration, registrationForToken, saveRegistrationDraft } from "./registration-service";
import { approveRegistration, submitRegistration } from "./registration-jobs";
import { discoverRegistrationWebsite } from "./registration-content";
import { dateTimeNowAsValue } from "../shared/dates";
import { HttpError } from "../shared/http-error";
import { preparedRegistrationSettings } from "./registration-defaults";
...
export async function discoverRegistration(req: Request, res: Response): Promise<void> {
  ...
  const discovered = await discoverRegistrationWebsite(saved.website, saved.group?.group_code);
  await registrations().updateOne({id: saved.id, state: RegistrationState.DRAFT, website: saved.website}, {$set: {...discovered, updatedAt: dateTimeNowAsValue()}});
  await readRegistration(req, res);
}
...
export async function listRegistrations(_req: Request, res: Response): Promise<void> {
  ...
  res.json(await Promise.all(rows.map(async row => ({
    ...
    siteHealth: await registrationSiteHealth(row)
  }))));
}
...
export async function retryRegistration(req: Request, res: Response): Promise<void> {
  const saved = await registrations().findOne({id: req.params.id, state: {$in: [RegistrationState.FAILED, RegistrationState.BROKEN, RegistrationState.REVIEW]}});
  if (!saved || saved.leaseUntil > dateTimeNowAsValue()) {
  ...
  await registrations().updateOne({id: saved.id, state: saved.state, leaseUntil: {$lte: dateTimeNowAsValue()}}, {$set: {
    ...
    importedAt: saved.reviewedAt ? saved.importedAt : null, reviewNotifiedAt: saved.reviewedAt ? saved.reviewNotifiedAt : null, error: null
  }});
  res.json({success: true});
}
