...
export function registrationActor(req: Request): string {
  const member = req.user as MemberCookie;
  return [member?.firstName, member?.lastName].filter(name => !!name?.trim()).join(" ").trim() || member?.userName || "An administrator";
}
