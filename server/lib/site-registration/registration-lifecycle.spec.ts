...
describe("site registration lifecycle", () => {
  ...
  it("starts an area registration when no group is chosen, using the area's directory entry", async () => {
    const state: {registration: StoredSiteRegistration | null} = {registration: null};
    const area = {...group, scope: "A", group_code: "AB", area_code: "AB", name: "Example Area"};
    const areaSettings = {...settings, approvedEmails: [{areaCode: "AB", groupCode: "", recipients: [{email: "area@example.com", name: "Area"}]}]};
    sandboxState.sandbox.stub(registrationStore, "registrations").returns(inMemoryRegistrations(state));
    sandboxState.sandbox.stub(registrationStore, "registrationSettings").resolves(areaSettings);
    sandboxState.sandbox.stub(registrationStore, "ensureRegistrationIndexes").resolves();
    sandboxState.sandbox.stub(groupsApi, "fetchRamblersGroupsFromApi").resolves([area, group] as any);
    sandboxState.sandbox.stub(registrationEmail, "sendRegistrationEmail").resolves();
    sandboxState.sandbox.stub(environmentsConfig, "findEnvironmentFromDatabase").resolves(null);

    await startRegistration({areaCode: "AB", groupCode: "", email: "area@example.com", plan: RegistrationPlan.LITE});
    expect(state.registration.group.scope).toBe("A");
    expect(state.registration.group.name).toBe("Example Area");
  ...
  it("takes a Lite registration from approved email through confirmation, review and invitation", async () => {
    ...
    const started = await startRegistration({areaCode: "AB", groupCode: "AB01", email: "committee@example.com", plan: RegistrationPlan.LITE});
