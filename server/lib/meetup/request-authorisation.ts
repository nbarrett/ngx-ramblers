...
export async function requestAuthorisation(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!meetupConfig?.clientId) {
      ...
      const requestAuthorisationUrl = `https://secure.meetup.com/oauth2/authorize?client_id=${encodeURIComponent(meetupConfig.clientId)}&response_type=code&redirect_uri=${encodeURIComponent(meetupConfig.clientRedirectUrl)}&state=meetup`;
