...
import { DomainEventDataWithFinished } from "../../../projects/ngx-ramblers/src/app/models/ramblers-upload-audit.model";
      ...
      try {
        ...
        if (handler) {
          if (request.type === EventType.TEST_STEP_REPORTER) {
            try {
              const parsed: DomainEventDataWithFinished = JSON.parse(request.data);
              const name = parsed.eventData?.details?.name || "(step)";
              const outcomeCode = parsed.eventData?.outcome?.code;
              const outcomeText = outcomeCode === 64 ? "SUCCESS" : `outcome:${outcomeCode}`;
              debugLog(`Serenity step: "${name}" → ${outcomeText}`);
            } catch {
              debugLog(`Serenity step: (unparseable payload)`);
            }
          } else if (request.type !== EventType.PING) {
            debugLog(`Message received for type: ${request.type}`);
          }
