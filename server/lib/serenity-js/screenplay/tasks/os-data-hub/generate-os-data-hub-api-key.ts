...
export class GenerateOsDataHubApiKey extends Interaction {
  ...
  async performAs(actor: UsesAbilities): Promise<void> {
    ...
    if (!projectName || !jobPath) {
      ...
      await this.openProject(native, projectName, jobPath, timeout);
      await this.ensureOsMapsApi(native, jobPath, timeout);
      const apiKey = await this.projectApiKey(native, timeout);
      fs.writeFileSync(path.join(jobPath, OS_DATA_HUB_API_KEY_FILE), JSON.stringify({projectName, apiKey}));
      debugLog("stored the API key for project %s (%d characters)", projectName, apiKey.length);
    }
  }

  private async openProject(native: NativePage, projectName: string, jobPath: string, timeout: number): Promise<void> {
  ...
  private async ensureOsMapsApi(native: NativePage, jobPath: string, timeout: number): Promise<void> {
