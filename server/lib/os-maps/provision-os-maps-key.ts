...
import { OS_DATA_HUB_API_KEY_MAX_WAIT_MS, OS_DATA_HUB_API_KEY_POLL_MS, OsDataHubApiKeyJobResult, OsDataHubApiKeyJobStatus } from "../../../projects/ngx-ramblers/src/app/models/os-data-hub-api-key.model";
...
export async function generateOsMapsApiKey(projectName: string, email: string, password: string, report: (message: string) => Promise<unknown>): Promise<string | null> {
...
export async function provisionOsMapsApiKey(environmentName: string, report: (message: string) => Promise<unknown> = async () => undefined): Promise<OsMapsKeyProvision> {
  ...
  if (!platform.email || !platform.password) {
    ...
    try {
      ...
      return generated
        ? {apiKey: generated, message: `Generated an OS Maps API key for project ${projectName}`}
