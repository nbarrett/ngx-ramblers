...
import {
  AlbumIndexSortField, ContentTemplateType, IndexContentType, IndexRenderMode, MigrationTemplateSourceType, NestedRowContentSource,
  NestedRowPackingBehavior, PageContent, PageContentType, REGISTRATION_INTRO_PATH, StringMatch
} from "../../../../../projects/ngx-ramblers/src/app/models/content-text.model";
import { REGISTRATION_TEMPLATE_YEAR, RegistrationMigrationTemplate } from "../../../../../projects/ngx-ramblers/src/app/models/site-registration.model";
...
export async function up(db: Db, _client: MongoClient): Promise<void> {
  if (process.env[Environment.PLATFORM_ADMIN_ENABLED] === "true") {
    await seedPlatformRegistration(db);
  } else {
    debugLog("Skipped: group registration lives on the platform site only.");
  }
}

async function seedPlatformRegistration(db: Db): Promise<void> {
  ...
  const pages = [
    {path: REGISTRATION_INTRO_PATH, contentText: "# Register your group or area\n\nCreate a free review site from your group's own public pages, photos and current Walks Manager programme. Your existing website is not changed, and your group decides whether to continue after reviewing the result. [Learn how registration works](/how-to/group-registration)."},
...
function albumIndex(contentPath: string, contentTypes: IndexContentType[], renderModes: IndexRenderMode[], field: AlbumIndexSortField) {
  return {
    contentPaths: [{contentPath, stringMatch: StringMatch.STARTS_WITH, maxPathSegments: 1}],
    excludePaths: [], columnOverrides: [], contentTypes, renderModes, indexMarkdown: "", autoTitle: false,
    showInParentIndex: true, minCols: 2, maxCols: 3, sortConfig: {field, direction: SortDirection.DESC}, entryOverrides: {}
  };
}

function photoTemplates(): PageContent[] {
  return [{
    path: RegistrationMigrationTemplate.PHOTOS_YEAR,
    rows: [{
      type: PageContentType.TEXT, maxColumns: 1, showSwiper: false,
      columns: [{columns: 12, contentText: `# ${REGISTRATION_TEMPLATE_YEAR}\n\nPhotos from our walks in ${REGISTRATION_TEMPLATE_YEAR}.`}]
    }, {
      type: PageContentType.ALBUM_INDEX, maxColumns: 3, showSwiper: false, columns: [], marginBottom: 3,
      albumIndex: {
        ...albumIndex("photos/2026", [IndexContentType.ALBUMS], [IndexRenderMode.MAP, IndexRenderMode.ACTION_BUTTONS], AlbumIndexSortField.EVENT_DATE),
        mapConfig: {
          mapHeight: 513, showControlsDefault: false, allowControlsToggle: false, autoFitBounds: true, provider: "os",
          osStyle: "Outdoor_27700", mapZoom: 10, clusteringEnabled: false, clusteringThreshold: 10
        }
      }
    }],
    migrationTemplate: {isTemplate: true, templateType: ContentTemplateType.MIGRATION_TEMPLATE, mappings: [],
      templateName: "Self-service photos year",
      templateDescription: `One page per year under Photos. ${REGISTRATION_TEMPLATE_YEAR} is replaced with the year, the index shows that year's albums, and the map is centred on the group or area.`}
  } as PageContent, {
    path: RegistrationMigrationTemplate.PHOTOS_INDEX,
    rows: [{
      type: PageContentType.ALBUM_INDEX, maxColumns: 3, showSwiper: false, columns: [], marginBottom: 3,
      albumIndex: albumIndex("photos", [IndexContentType.INDEX_PAGES, IndexContentType.PAGES], [IndexRenderMode.ACTION_BUTTONS], AlbumIndexSortField.TITLE)
    }],
    migrationTemplate: {isTemplate: true, templateType: ContentTemplateType.MIGRATION_TEMPLATE, mappings: [],
      templateName: "Self-service photos index",
      templateDescription: "Added below the imported Photos page text, listing the year pages."}
  } as PageContent];
}
