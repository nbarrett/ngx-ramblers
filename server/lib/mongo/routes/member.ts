...
router.post("/bulk-delete", authConfig.authenticate(), async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({action: `Bulk deletion of ${pluraliseWithCount(memberIds.length, "member")}`, response: result.deletionResponses});
  } catch (error: any) {
...
router.post("/apply-post-send-actions", authConfig.authenticate(), async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({action: `Post-send actions applied to ${pluraliseWithCount(memberIds.length, "member")}`, response: result});
  } catch (error: any) {
