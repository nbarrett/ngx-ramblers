...
describe("site registration schemas", () => {
  it("keeps every registration setting including disabled committee email validation", () => {
    const settings = new siteRegistrationConfig({
      ...
      value: {
        ...
        approvedEmails: [{areaCode: "AB", areaName: "Example Area", groupCode: "AB01", groupName: "Example Group", recipients: [{email: "committee@example.org", name: "Committee Secretary"}]}]
      }
    }).toObject();
  ...
  it("defines every stored registration attribute", () => {
    const requiredPaths = [
      ...
      "reviewedAt", "reviewNotifiedAt", "invitedAt", "leaseUntil", "leaseOwner", "history", "discoveryProgress"
    ];
