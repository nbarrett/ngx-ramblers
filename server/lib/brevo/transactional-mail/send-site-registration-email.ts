...
import {
  BuiltInProcessMappings, NotificationConfig, SendPurpose, SendSmtpEmailRequest
...
import { buildSubject } from "./send-member-bulk-load-digest-email";
...
export async function sendRegistrationEmail(settings: RegistrationSettings, type: RegistrationEmailType, recipient: string, values: RegistrationEmailParameters): Promise<void> {
  ...
  if (!configured?.body) {
    ...
    const params = {
      messageMergeFields: {
        subject: "", BANNER_IMAGE_SOURCE: "", ADDRESS_LINE: "Hello,", BODY_CONTENT: "", BODY_CONTENT_BOTTOM: "",
    ...
    const request: SendSmtpEmailRequest = {
      ...
      params, body: configured.body, templateName: configured.templateName || "fully-automated-text-body",
