...
describe("campaign attachment URLs", () => {
  ...
  it("reads the extension from the last path segment", () => {
    expect(attachmentExtensionFromUrl("https://www.example.co.uk/api/calendar/meeting/ngx-room.ics")).toEqual("ics");
    expect(attachmentExtensionFromUrl("https://www.example.co.uk/api/aws/s3/email-attachments/file.pdf")).toEqual("pdf");
  });
  ...
  it("extracts the meeting room from a calendar attachment URL", () => {
    ...
    expect(meetingRoomFromAttachmentUrl("https://www.example.co.uk/api/calendar/meeting/ngx-room.ics"))
      .toEqual("ngx-room");
  });

  it("returns null when the URL is not a meeting calendar", () => {
    expect(meetingRoomFromAttachmentUrl("https://www.example.co.uk/api/aws/s3/email-attachments/notes.pdf")).toEqual(null);
  });
