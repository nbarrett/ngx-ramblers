...
export async function campaignQueueSummary(): Promise<BrevoCampaignQueueSummary> {
  ...
  const completionTo = dateTimeNow().endOf("day").toISO()!;
  ...
  const pendingCampaigns = (pendingResponse.body?.campaigns ?? [])
    .filter(isNgxCampaign)
    .map(campaignProgress)
    .filter(campaign => campaign.remaining > 0);
  const completedCampaigns = (completedResponse.body?.campaigns ?? [])
    .filter(isNgxCampaign)
    .map(campaignProgress);
