...
async function inboundRouterSecret(): Promise<string | null> {
  const environmentSecret = envConfig.value(Environment.NGX_INBOUND_SECRET)?.trim();
  if (environmentSecret) {
    return environmentSecret;
  }
  const environmentsConfig = await configuredEnvironments();
  return environmentsConfig.secrets?.[Environment.NGX_INBOUND_SECRET]?.trim() || null;
}

async function ensureRouterWorker(cloudflareConfig: CloudflareConfig): Promise<string> {
  const secret = await inboundRouterSecret();
  if (!secret) {
    throw new HttpError(400, `No shared inbound router secret is configured (${Environment.NGX_INBOUND_SECRET}). Set it in the platform admin shared secrets and redeploy this site before routing mail to inboxes.`, "RouterSecretMissing");
  }
  await uploadWorkerScript(cloudflareConfig, ROUTER_WORKER_NAME, generateRouterWorkerScript());
  await uploadWorkerSecret(cloudflareConfig, ROUTER_WORKER_NAME, Environment.NGX_INBOUND_SECRET, secret);
...
async function planCatchAllRule(
  ...
  if (action === CatchAllAction.WORKER) {
    ...
    if (webhookContext?.secret) {
      await uploadWorkerSecret(cloudflareConfig, catchAllScriptName, Environment.NGX_INBOUND_SECRET, webhookContext.secret);
    }
...
export async function putCatchAllRule(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
...
export async function postCatchAllRouterRedeploy(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
  const nsConfig = await nonSensitiveCloudflareConfig();
  await assertCatchAllOwnedByThisSite(cloudflareConfig, nsConfig.baseDomain || "");
  const scriptName = await ensureRouterWorker(cloudflareConfig);
...
export async function getDestinationAddresses(req: Request, res: Response): Promise<void> {
...
export async function postDestinationAddress(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
...
export async function postDestinationAddressResend(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
...
export async function deleteDestinationAddressById(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
...
export async function getWorkerRecipients(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
...
export async function getWorkerInfo(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
  const scriptContent = await fetchWorkerScriptContent(cloudflareConfig, req.params.scriptName);
...
export async function postWorker(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
  const nsConfig = await nonSensitiveCloudflareConfig();
  ...
  if (webhookContext?.secret) {
    await uploadWorkerSecret(cloudflareConfig, scriptName, Environment.NGX_INBOUND_SECRET, webhookContext.secret);
  }
...
export async function renameWorker(req: Request, res: Response): Promise<void> {
  const cloudflareConfig = await configuredCloudflare();
...
export async function postEmailRoutingLogs(req: Request, res: Response): Promise<void> {
...
export async function postWorkerLogs(req: Request, res: Response): Promise<void> {
...
export async function getMxRecords(req: Request, res: Response): Promise<void> {
  const nsConfig = await nonSensitiveCloudflareConfig();
...
export async function postAuthRecords(req: Request, res: Response): Promise<void> {
  const nsConfig = await nonSensitiveCloudflareConfig();
  if (!nsConfig.configured || !nsConfig.baseDomain) {
    throw new HttpError(400, "Cloudflare not configured or baseDomain not available");
  }
...
export { handleInboundMime, handleInboundInbox };
