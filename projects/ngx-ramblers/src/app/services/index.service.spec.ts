...
import { AlbumIndexSortField, ContentPathMatch, PageContent, PageContentColumn, PageContentRow, PageContentType, StringMatch } from "../models/content-text.model";
...
describe("IndexService", () => {
    ...
    describe("depthLimitedRegex", () => {
        ...
        it("should match only volume-level paths with maxPathSegments=1", () => {
            ...
            expect(regex.test("walks/scrapbook/by-year/2015/volume-11")).toBe(true);
            expect(regex.test("walks/scrapbook/by-year/2015/volume-12")).toBe(true);
            expect(regex.test("walks/scrapbook/by-year/2015/volume-11/walk-139")).toBe(false);
            expect(regex.test("walks/scrapbook/by-year/2015/volume-11/walk-139/detail")).toBe(false);
            expect(regex.test("walks/scrapbook/by-year/2015")).toBe(false);
        });
        ...
        it("should allow up to 2 levels deep with maxPathSegments=2", () => {
            ...
            expect(regex.test("gallery/2025")).toBe(true);
            expect(regex.test("gallery/2025/walk-album")).toBe(true);
            expect(regex.test("gallery/2025/walk-album/detail")).toBe(false);
        });
