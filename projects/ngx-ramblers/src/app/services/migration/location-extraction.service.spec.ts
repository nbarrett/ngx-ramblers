...
describe("MigrationLocationExtractionService", () => {
    ...
    describe("real world examples", () => {
        it("should extract multiple locations from narrative text", () => {
            ...
            const cernesFarm = requireValue(locations.find(l => l.value.includes("Cernes Farm")), `Expected to find Cernes Farm in: ${JSON.stringify(locations)}`);
            expect(cernesFarm?.type).toBe(GeocodeMatchType.PLACE_NAME);
            expect(cernesFarm?.value).toMatch(/Cernes Farm/);

            const tonbridgeCastle = requireValue(locations.find(l => l.value.includes("Tonbridge Castle")), `Expected to find Tonbridge Castle in: ${JSON.stringify(locations)}`);
            expect(tonbridgeCastle?.type).toBe(GeocodeMatchType.PLACE_NAME);

            requireValue(locations.find(l => l.value.includes("Edenbridge")), `Expected to find Edenbridge in: ${JSON.stringify(locations)}`);

            const penshurst = requireValue(locations.find(l => l.value.includes("Penshurst")), `Expected to find Penshurst in: ${JSON.stringify(locations)}`);
        ...
        it("should extract Cudham and Christmas Tree Farm walk and extract the starting Grid ref as TQ446597 then use this", () => {
            ...
            expect(locations.length).toBeGreaterThanOrEqual(2);
            const gridRef = requireValue(locations.find(l => l.type === GeocodeMatchType.GRID_REFERENCE), `Expected to find grid reference in: ${JSON.stringify(locations)}`);
        ...
        it("should extract The 153-mile (246 km) Saxon Shore Way from Gravesend to Hastings", () => {
            ...
            expect(locations.length).toBeGreaterThanOrEqual(2);
            const gravesend = requireValue(locations.find(l => l.value.includes("Gravesend")), `Expected to find Gravesend in: ${JSON.stringify(locations)}`);
            expect(gravesend?.type).toBe(GeocodeMatchType.PLACE_NAME);
            expect(gravesend?.value).toMatch(/Gravesend/);

            const hastings = requireValue(locations.find(l => l.value.includes("Hastings")), `Expected to find Hastings in: ${JSON.stringify(locations)}`);
        ...
        it("should extract Thames Estuary and Beachy Head as start/end locations", () => {
            ...
            requireValue(gravesend || beachyHead || eastbourne || thamesEstuary, `Expected to find at least one major location in: ${JSON.stringify(locations)}`);
        });
        ...
        it("should extract Starting point at Chipstead", () => {
            ...
            expect(locations.length).toBeGreaterThanOrEqual(1);
            const chipstead = requireValue(locations.find(l => l.value.includes("Chipstead")), `Expected to find Chipstead in: ${JSON.stringify(locations)}`);
      ...
      The Darent Valley Path starts alternatively at Sevenoaks Station or Chipstead and finishes on the bank of the Thames just north of Dartford.`;
            ...
            if (startLocations.length < 2) {
                throw new Error(`Expected multiple start locations in: ${JSON.stringify(locations)}`);
            }

            const best = service.bestLocation(locations);
            expect(best).toBeDefined();
            expect(best?.context).toBe("start location");
            if (!best?.value?.includes("Sevenoaks Station")) {
                throw new Error(`Expected 'Sevenoaks Station' but got '${best?.value}'`);
            }
            expect(best?.value).toContain("Sevenoaks Station");
        });
...
Kent Ramblers have published a new guide to three of west Kent's river valley walks, including the Darent Valley Path:`;
            ...
            if (locations.length < 3) {
                throw new Error(`All locations: ${JSON.stringify(locations)}`);
            }
            ...
            requireValue(startLocations.find(l => l.value === "Chipstead"), "Expected to find 'Chipstead' in start locations");

            requireValue(startLocations.find(l => l.value.includes("Sevenoaks Station")), `Expected to find 'Sevenoaks Station' in: ${JSON.stringify(startLocations)}`);

            const endLocations = locations.filter(l => l.context === "end location");
            if (endLocations.length < 1) {
                throw new Error(`Expected end location in: ${JSON.stringify(locations)}`);
            }

            requireValue(endLocations.find(l => l.value.includes("Thames") || l.value.includes("Dartford")), `Expected to find Thames/Dartford in end locations: ${JSON.stringify(endLocations)}`);

            const best = service.bestLocation(locations);
            expect(best).toBeDefined();
            expect(best?.context).toBe("start location");
            if (!best?.value?.includes("Sevenoaks Station")) {
                throw new Error(`Expected 'Sevenoaks Station' but got '${best?.value}'. All start locations: ${JSON.stringify(startLocations)}`);
            }
            expect(best?.value).toContain("Sevenoaks Station");
        });
