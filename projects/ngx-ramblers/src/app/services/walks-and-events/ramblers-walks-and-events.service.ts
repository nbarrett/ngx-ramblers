...
import { enumKeyValues, enumValues } from "../../functions/enums";
...
export class RamblersWalksAndEventsService {
  ...
  private logger: Logger = inject(LoggerFactory).createLogger("RamblersWalksAndEventsService", NgxLoggerLevel.INFO);
  ...
  async socialEventForId(socialEventId: string): Promise<ExtendedGroupEvent> {
  ...
  updateWalksWithRamblersWalkData(ramblersWalksResponses: RamblersWalkResponse[], walks: ExtendedGroupEvent[]): Promise<LocalAndRamblersWalk[]> {
    ...
    ramblersWalksResponses.forEach((ramblersWalksResponse: RamblersWalkResponse) => {
      const walkMatchedByDate: ExtendedGroupEvent = walks.find(walk => this.dateUtils.asString(walk.groupEvent.start_date_time, undefined, "dddd, Do MMMM YYYY") === ramblersWalksResponse.startDate);
    ...
    if (unreferencedUrls.length > 0) {
      ...
      unreferencedUrls.map((url: string) => {
        const walkMatchedByUrl: ExtendedGroupEvent = walks.find(walk => walk.groupEvent.url === url);
        if (walkMatchedByUrl) {
  ...
  private integrate(walks: ExtendedGroupEvent[], ramblersWalksResponses: RamblersWalkResponse[]): LocalAndRamblersWalk[] {
  ...
  private notMatchedByIdOrUrl(walkMatchedByDate: ExtendedGroupEvent, ramblersWalksResponse: RamblersWalkResponse): boolean {
    return walkMatchedByDate.groupEvent.url !== ramblersWalksResponse.url || walkMatchedByDate.groupEvent.id !== ramblersWalksResponse.id;
  }
  ...
  public toWalkExport(localAndRamblersWalk: LocalAndRamblersWalk): WalkExport {
    ...
    if (isEmpty(walk)) {
      ...
      this.logger.off("validateWalk:walk.groupEvent.end_date_time:", walk.groupEvent.end_date_time, "walk.groupEvent.start_date_time:", walk.groupEvent.start_date_time);
  ...
  startingLocationDetails(walk: ExtendedGroupEvent) {
    return walk.groupEvent.start_location?.description ? `${this.NEAREST_TOWN_PREFIX}${walk.groupEvent.start_location?.description}` : "";
  }
  ...
  walkType(walk: ExtendedGroupEvent): string {
    return walk.groupEvent.shape || WalkType.CIRCULAR;
  }
  ...
  async walkToWalkUploadRow(walk: ExtendedGroupEvent): Promise<WalkUploadRow> {
    ...
    csvRecord[WalkUploadColumnHeading.EST_FINISH_TIME] = this.walkFinishTimeOrDefault(walk, this.walksConfig?.milesPerHour);
    ...
    csvRecord[WalkUploadColumnHeading.DIFFICULTY] = this.asString(walk.groupEvent.difficulty.description);
  ...
  public walkFinishTimeOrDefault(walk: ExtendedGroupEvent, milesPerHour: number): string {
    const endTimeValid = walk?.groupEvent?.end_date_time?.length > 5;
    this.logger.info("walkFinishTimeOrDefault:groupEvent.end_date_time:", walk.groupEvent.end_date_time, "endTimeValid:", endTimeValid, "of type", typeof walk?.groupEvent?.end_date_time);
    if (endTimeValid) {
      return walk.groupEvent.end_date_time;
    } else {
      return this.walkFinishTime(walk, milesPerHour);
    }
  }

  public walkFinishTime(walk: ExtendedGroupEvent, milesPerHour?: number): string {
    return this.dateUtils.isoDateTimeString(this.dateUtils.startTimeAsValue(walk) + this.dateUtils.durationInMsecsForDistanceInMiles(walk.groupEvent.distance_miles, walk.fields.milesPerHour || milesPerHour));
  }

  walkStartTime(walk: ExtendedGroupEvent): string {
    return walk.groupEvent.start_date_time ? this.dateUtils.asString(this.dateUtils.startTimeAsValue(walk), null, "HH:mm") : "";
  }

  startingGridReference(walk: ExtendedGroupEvent): string {
    return this.walkDisplayService.gridReferenceFrom(walk?.groupEvent?.start_location);
  }


  startingPostcode(walk: ExtendedGroupEvent): string {
    return walk?.groupEvent?.start_location?.postcode || "";
  }

  walkFinishGridReference(walk: ExtendedGroupEvent): string {
    return walk?.groupEvent?.end_location?.grid_reference_10 || "";
  }

  walkFinishPostcode(walk: ExtendedGroupEvent): string {
    return this.walkDisplayService.gridReferenceFrom(walk?.groupEvent?.end_location) ? "" : walk?.groupEvent?.end_location?.postcode || "";
  }

  walkDate(walk: ExtendedGroupEvent, format: string): string {
    return this.dateUtils.asString(walk.groupEvent.start_date_time, null, format);
  }
  ...
  featureTrueOrFalseSelection(featureCode: Feature, walk: ExtendedGroupEvent): string {
    return this.featureSelected(featureCode, walk) ? "TRUE" : "FALSE";
  }

  featureSelected(featureCode: Feature, walk: ExtendedGroupEvent): boolean {
    return walk.groupEvent.accessibility.some(feature => feature.code === featureCode);
  }
  ...
  toExtendedGroupEvent(groupEvent: GroupEvent): ExtendedGroupEvent {
    const contact: LocalContact = this.localContact(groupEvent);
