...
export class GroupEventService {
  ...
  private currentPreviousData(eventsLatestFirst: WalkEvent[], extendedGroupEvent: ExtendedGroupEvent, basedOnUnsavedData: boolean): CurrentPreviousData {
    if (basedOnUnsavedData) {
      ...
      const previousData = normaliseWalkEventSnapshot(latestEvent?.data);
      ...
      const currentData = normaliseWalkEventSnapshot(latest2Events.length === 2 ? latest2Events[0]?.data || null : latest2Events[1]?.data || null);
      const previousData = normaliseWalkEventSnapshot(latest2Events.length === 2 ? latest2Events[1]?.data : null);
