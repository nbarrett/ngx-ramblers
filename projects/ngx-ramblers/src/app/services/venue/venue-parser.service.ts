...
export class VenueParserService {
  ...
  private inferVenueType(text: string, name: string): string {
    const lowerText = (text + " " + name).toLowerCase();

    if (/\b(car park|car-park|carpark|parking|layby|lay-by)\b/.test(lowerText)) {
      return "car park";
    }
    if (/\b(cafe|coffee|tea room|tearoom)\b/.test(lowerText)) {
      return "cafe";
    }
    if (/\b(restaurant|bistro|diner|eatery|kitchen)\b/.test(lowerText)) {
      return "restaurant";
    }
    if (/\b(village hall|community hall|town hall|memorial hall|parish hall|church hall)\b/.test(lowerText)) {
      return "hall";
    }
    if (/\b(pub|inn|tavern|arms|head|bell|crown|swan|bull|lion|rose|anchor|oak|fox|bear|eagle|plough|brewery|ale|beer|robin|horse|hare|stag|hart|dog|cock|hen|pheasant|falcon|hawk|raven|magpie|pig|boar|lamb|sheep|goat|badger|otter|deer|buck)\b/.test(lowerText)) {
      return "pub";
    }
    if (/\b(church|chapel|cathedral|minster)\b/.test(lowerText)) {
      return "church";
    }
    if (/\b(hall|centre|center|community)\b/.test(lowerText)) {
      return "hall";
    }
