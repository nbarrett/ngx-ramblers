...
import { formatEmailAddress } from "./email-addresses";
...
export function approvalRecipientLabels(entry: RegistrationEmailApproval): string {
  return approvalRecipients(entry).map(recipient => formatEmailAddress(recipient)).join(", ");
}

export function approvalGroupLabel(entry: RegistrationEmailApproval): string {
  const name = (entry.groupName || "").trim();
  return name ? `${name} (${entry.groupCode})` : entry.groupCode;
}
...
export function registrationSettingsProblem(settings: RegistrationSettings): string {
  ...
  } else if (!isArray(settings?.approvedEmails) || !settings.approvedEmails.every(entry => isString(entry.groupCode) && isArray(entry.recipients))) {
