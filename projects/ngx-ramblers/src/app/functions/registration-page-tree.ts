...
export function registrationPageTree(pages: RegistrationPage[]): SitemapNode[] {
  const byPath = pages.reduce((index, page) => index.set(page.path, {key: page.path, title: page.title, href: page.url, selected: page.selected, detail: registrationPageDetail(page), children: []}), new Map<string, SitemapNode>());
...
function pageMatchesRole(page: RegistrationPage, synonyms: string[]): boolean {
  const leaf = (page.path.split("/").pop() || "").toLowerCase().replace(/-/g, " ");
  const title = page.title.trim().toLowerCase();
  return synonyms.some(synonym => leaf === synonym || title === synonym);
}
...
export function standardiseRegistrationPageNames(pages: RegistrationPage[]): RegistrationPage[] {
  return NAVBAR_ROLES.reduce((current, role) => {
    ...
    const match = titled.find(page => !page.parentPath && page.path !== role.path && pageMatchesRole(page, role.synonyms));
...
export function isRegistrationNavLabel(title: string): boolean {
  const words = title.trim().split(/\s+/).filter(Boolean);
  return words.length > 0 && words.length <= 3 && !/^[a-z]/.test(title.trim()) && !/\?$/.test(title.trim()) && !/^(our|the|this|here|how|more|details)\b/i.test(title.trim());
}

export function assembleRegistrationPages(pages: RegistrationPage[], hasWalks = false, hasSocialEvents = false): RegistrationPage[] {
  const flattened = pages.filter(page => isRegistrationFeaturePath(page.path) || page.path === RegistrationNavbarPath.HOME || isRegistrationNavLabel(page.title))
    .map(page => ({...page, path: page.path.split("/").pop() || page.path, parentPath: null as string}));
