import { buildAccessibilityCriteria, buildAdvancedSearchCriteria, buildDaysOfWeekCriteria, buildDifficultyCriteria, buildDistanceRangeCriteria, buildFacilitiesCriteria, buildFreeOnlyCriteria, buildGroupCodesCriteria, buildLeaderIdsCriteria, buildProximityCriteria } from "./advanced-search-criteria-builder";
...
describe("Advanced Search Criteria Builder", () => {
    let mockDateUtils: {
        daysOfWeek: ReturnType<typeof vi.fn>;
        mongoDayOfWeekFromName: ReturnType<typeof vi.fn>;
    };
    let dateUtils: DateUtilsService;

    beforeEach(() => {
        mockDateUtils = {
            daysOfWeek: vi.fn().mockName("DateUtilsService.daysOfWeek"),
            mongoDayOfWeekFromName: vi.fn().mockName("DateUtilsService.mongoDayOfWeekFromName")
        };
        mockDateUtils.daysOfWeek.mockReturnValue([
            "Sunday",
            "Monday",
            "Tuesday",
            "Wednesday",
        ...
        mockDateUtils.mongoDayOfWeekFromName.mockImplementation((dayName: string) => {
            const days: {
                [key: string]: number;
            } = {
                "Sunday": 1,
        ...
        dateUtils = mockDateUtils as unknown as DateUtilsService;
    });
    ...
    describe("buildDaysOfWeekCriteria", () => {
        it("should build criteria for selected days of week", () => {
            ...
            const criteria = buildDaysOfWeekCriteria(daysOfWeek, dateUtils);
        ...
        it("should return null when no valid days provided", () => {
            const criteria = buildDaysOfWeekCriteria([], dateUtils);
