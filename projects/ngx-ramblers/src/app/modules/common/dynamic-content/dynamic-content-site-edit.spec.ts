...
import { BreakStopCondition, NestedRowContentSource, NestedRowMappingConfig, NestedRowPackingBehavior } from "../../../models/content-text.model";
import { assignDeep } from "../../../functions/object-utils";

describe("updateColumnNestedRowMapping", () => {
    let mockComponent: any;
    let mockEnsureColumnMapping: Mock;

    beforeEach(() => {
        mockEnsureColumnMapping = vi.fn();

        mockComponent = {
            isMigrationTemplateSelected: vi.fn().mockReturnValue(true),
            ensureColumnMapping: mockEnsureColumnMapping
        };
    });

    function updateColumnNestedRowMapping(rowIndex: number, columnIndex: number, updates: Partial<NestedRowMappingConfig>, nestedRowIndex?: number, nestedColumnIndex?: number) {
    ...
    it("should return early if migration template is not selected", () => {
        mockComponent.isMigrationTemplateSelected.mockReturnValue(false);
        const colMapping = { nestedRowMapping: {} };
        mockEnsureColumnMapping.mockReturnValue(colMapping);
