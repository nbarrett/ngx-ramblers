    ...
    @if (row?.areaMap) {
      ...
      <div class="row align-items-end mb-3 g-3">
        <div class="col-md-6 col-lg-3">
          <div class="form-group">
            <label class="form-label" for="center-lat-{{id}}">Center Latitude</label>
            <div class="editor-slider-group">
              ...
              <span class="text-muted editor-slider-value">{{ centerLat | number:"1.4-4" }}</span>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
          <div class="form-group">
            <label class="form-label" for="center-lng-{{id}}">Center Longitude</label>
            <div class="editor-slider-group">
              <input type="range"
                     id="center-lng-{{id}}"
                     class="form-range editor-slider"
                     ...
                     [(ngModel)]="centerLng"
                     (ngModelChange)="updateMapCenter()">
              <span class="text-muted editor-slider-value">{{ centerLng | number:"1.4-4" }}</span>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
          <div class="form-group">
            ...
            <div class="editor-slider-group">
              ...
              <span class="text-muted editor-slider-value">{{ row.areaMap.mapZoom | number:"1.2-2" }}</span>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
          <div class="form-group">
            ...
            <div class="editor-slider-group">
              ...
              <span class="text-muted editor-slider-value">{{ row.areaMap.mapHeight }}px</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="form-group">
            ...
            <div class="editor-slider-group">
              <input type="range"
                     ...
                     (ngModelChange)="broadcastChange()">
              <span class="text-muted editor-slider-value">{{ row.areaMap.opacityNormal }}</span>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
          <div class="form-group">
            ...
            <div class="editor-slider-group">
              <input type="range"
                     ...
                     (ngModelChange)="broadcastChange()">
              <span class="text-muted editor-slider-value">{{ row.areaMap.opacityHover }}</span>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
          <div class="form-group">
            ...
            <div class="editor-slider-group">
              <input type="range"
                     ...
                     (ngModelChange)="broadcastChange()">
              <span class="text-muted editor-slider-value">{{ row.areaMap.textOpacity }}</span>
            </div>
          </div>
        </div>
      </div>
...
export class DynamicContentSiteEditAreaMapComponent implements OnInit {
  ...
  private subscribeToCmsChanges() {
    this.cmsService.settingsChanges().subscribe(settings => {
      if (!settings) return;

      if (isNumber(settings.mapZoom) && this.row.areaMap) {
        this.row.areaMap.mapZoom = settings.mapZoom;
      }

      if (settings.mapCenter && isArray(settings.mapCenter)) {
        this.centerLat = settings.mapCenter[0];
        this.centerLng = settings.mapCenter[1];
        if (this.row.areaMap) {
          this.row.areaMap.mapCenter = settings.mapCenter;
        }
      }
    });
  }
  ...
  updateMapCenter() {
    if (this.row.areaMap && Number.isFinite(this.centerLat) && Number.isFinite(this.centerLng)) {
      ...
      this.cmsService.updateSetting("mapCenter", [this.centerLat, this.centerLng]);
      this.broadcastChange();
    }
  }
  ...
  onZoomChange() {
    if (this.row.areaMap) {
      this.cmsService.updateSetting("mapZoom", this.row.areaMap.mapZoom);
    }
    this.broadcastChange();
  }
  ...
  protected broadcastChange() {
    if (this.row.areaMap) {
      this.cmsService.updateSettings(this.row.areaMap);
    }
