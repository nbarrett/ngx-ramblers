...
const TREE_SIZE_KEY = "site-map-list-width";
const DEFAULT_TREE_SIZE = 420;

@Component({
  selector: "app-site-map-view",
  styleUrls: ["./site-map-view.sass"],
  imports: [NgTemplateOutlet, FormsModule, RouterLink, FontAwesomeModule, TooltipModule, ResizerComponent],
  template: `
    @if (showFilter) {
    ...
    @if (displayRoots.length) {
      <div [class.site-map-with-preview]="showPreview" [style.--site-map-list-width.px]="showPreview ? treeSize : null">
      <div class="site-map-list-pane">
      ...
      @if (showPreview) {
        <app-resizer class="site-map-resizer" subtle [variant]="ResizerVariant.BAR"
                     [orientation]="ResizerOrientation.HORIZONTAL"
                     [size]="treeSize" [minSize]="minTreeSize" [maxSize]="maxTreeSize"
                     resizeHint="Drag to change the width of the list"
                     (sizeChange)="onTreeSizeChange($event)" (resizeEnd)="persistTreeSize()"/>
      ...
      @for (node of nodes; track node.key; let index = $index) {
        <li class="site-map-node">
          <div class="site-map-row" [class.site-map-omitted]="selectable && node.selected === false">
            ...
            @if (showPreview) {
              <button type="button" class="site-map-link site-map-link-button" [class.site-map-focused]="focused?.key === node.key" (click)="focusAndReveal(node)" [innerHTML]="highlight(node.title)"></button>
            } @else if (node.href) {
          ...
          @if (node.detail && focused?.key === node.key) {
            <div class="site-map-detail">{{ node.detail }}</div>
          }
...
export class SiteMapViewComponent {
  ...
  readonly ResizerOrientation = ResizerOrientation;
  readonly ResizerVariant = ResizerVariant;
  readonly minTreeSize = 220;
  readonly maxTreeSize = 900;
  storageUnavailable = false;
  treeSize = this.storedTreeSize();
  ...
  @Output() focusChange = new EventEmitter<SitemapNode>();

  @Input() set roots(value: SitemapNode[]) {
    const previousRootKeys = this.rootKeys(this.treeRoots);
    this.treeRoots = value || [];
    this.onFilterChange(this.filter);
    if (this.viewMode === SiteMapViewMode.TREE && this.rootKeys(this.treeRoots) !== previousRootKeys) {
  ...
  onFilterChange(value: string): void {
    ...
    if (query.length > 0) {
      this.expandAll(this.filteredRoots);
    }
  }

  focusAndReveal(node: SitemapNode): void {
    this.focus(node);
  ...
  private expandAll(nodes: SitemapNode[]): void {
    nodes.forEach(node => {
      if (node.children.length) {
        this.expanded.add(node.key);
  ...
  private rootKeys(nodes: SitemapNode[]): string {
  ...
  onTreeSizeChange(size: number): void {
    this.treeSize = size;
  }

  persistTreeSize(): void {
    if (!this.storageUnavailable) {
      try {
        window.localStorage.setItem(TREE_SIZE_KEY, String(Math.round(this.treeSize)));
      } catch (error) {
  ...
  private storedTreeSize(): number {
    try {
      const stored = Number(window.localStorage.getItem(TREE_SIZE_KEY));
      return Number.isFinite(stored) && stored >= this.minTreeSize ? Math.min(stored, this.maxTreeSize) : DEFAULT_TREE_SIZE;
    } catch (error) {
      return DEFAULT_TREE_SIZE;
    }
  }

  pageCount(node: SitemapNode): number {
    return 1 + node.children.reduce((total, child) => total + this.pageCount(child), 0);
  }
  ...
  private nodeKeys(nodes: SitemapNode[]): string {
    return nodes.flatMap(node => [node.key, this.nodeKeys(node.children)]).sort().join("|");
  }
