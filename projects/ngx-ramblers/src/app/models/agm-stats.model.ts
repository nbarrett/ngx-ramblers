...
export enum AgmStatsDetailColumnKind {
  TEXT = "text",
  NUMBER = "number",
  CURRENCY = "currency"
}

export interface AgmStatsDetailColumn {
  label: string;
  kind?: AgmStatsDetailColumnKind;
}

export interface AgmStatsDetailLink {
  text: string;
  href: string;
}

export type AgmStatsDetailCell = string | number | null | AgmStatsDetailLink;

export interface AgmStatsDetailTable {
  title: string;
  columns: AgmStatsDetailColumn[];
  rows: AgmStatsDetailCell[][];
}
...
export interface AgmStatsEmailData {
  ...
  introText?: string;
  details?: Partial<Record<AgmStatsEmailSection, AgmStatsDetailTable[]>>;
}
