...
import { faArrowDownWideShort, faArrowLeft, faArrowUpWideShort, faBell, faBellSlash, faChevronDown, faChevronRight, faEnvelope, faEnvelopeOpen, faFilter, faInbox, faListCheck, faReply, faReplyAll, faRotateRight, faSearch, faTableColumns, faTableList, faTrash, faTriangleExclamation } from "@fortawesome/free-solid-svg-icons";
  ...
  template: `
    <app-page pageTitle="Email inbox">
      ...
      <div panelControls class="d-flex gap-2 align-items-center flex-grow-1 inbox-toolbar">
          ...
          <div class="ms-auto d-flex flex-wrap gap-2 inbox-toolbar-actions" [class.inbox-actions-odd]="mobileActionButtonsOdd">
          @if (mobile && mobileShowDetail) {
            <button class="btn btn-quiet d-flex align-items-center justify-content-center gap-1 text-nowrap" type="button" (click)="backToList()" tooltip="Back to inbox">
              <fa-icon [icon]="faArrowLeft"/>Inbox
            </button>
          }
          @if (threads.length > 0) {
            <button type="button" class="btn btn-quiet d-flex align-items-center justify-content-center gap-1 text-nowrap" [class.active]="readFilter === InboxReadFilter.UNREAD"
                    (click)="toggleUnreadFilter()"
                    [tooltip]="readFilter === InboxReadFilter.UNREAD ? 'Showing unread only — click to show all' : 'Show unread only'">
              <fa-icon [icon]="faFilter"/>{{ threadListUnreadCount }} unread
            </button>
          }
          @if (!mobile) {
            <button class="btn btn-quiet d-flex align-items-center justify-content-center gap-1 text-nowrap" type="button" (click)="toggleLayout()" [tooltip]="stackedLayout ? 'Switch to side-by-side view' : 'Switch to stacked view'">
          ...
          @if ((pushStatus$ | async); as pushStatus) {
            @if (pushStatus.supported) {
              @if (pushStatus.subscribed) {
                <button class="btn btn-quiet d-flex align-items-center justify-content-center gap-1 text-nowrap" type="button" (click)="disableBrowserNotifications()" [disabled]="busy" tooltip="Stop showing browser notifications for new inbox messages">
                  <fa-icon [icon]="faBellSlash"/>Notifications
                </button>
              } @else if (pushStatus.permission !== 'denied') {
                <button class="btn btn-quiet d-flex align-items-center justify-content-center gap-1 text-nowrap" type="button" (click)="enableBrowserNotifications()" [disabled]="busy" tooltip="Get a desktop or phone notification when new inbox mail arrives">
                  <fa-icon [icon]="faBell"/>Notifications
                </button>
              }
            }
          }
          <button class="btn btn-quiet d-flex align-items-center justify-content-center gap-1 text-nowrap" type="button" (click)="refresh()" [disabled]="busy" tooltip="Refresh the inbox">
...
export class InboxComponent implements OnInit, OnDestroy {
  ...
  private latestPushStatus: InboxPushSubscriptionStatus | null = null;
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.updateMobile();
    this.restoreLayout();
    this.subscriptions.push(this.pushStatus$.subscribe(status => this.latestPushStatus = status));
    await this.refresh();
  ...
  get notificationsButtonVisible(): boolean {
    return !!this.latestPushStatus?.supported
      && (this.latestPushStatus.subscribed || this.latestPushStatus.permission !== "denied");
  }

  get mobileActionButtonsOdd(): boolean {
    const count = (this.mobile && this.mobileShowDetail ? 1 : 0)
      + (this.threads.length > 0 ? 1 : 0)
      + (this.notificationsButtonVisible ? 1 : 0)
      + 1;
    return count % 2 === 1;
  }
