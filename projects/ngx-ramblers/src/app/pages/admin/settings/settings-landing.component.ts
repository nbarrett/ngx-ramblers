...
export class SettingsLandingComponent implements OnInit, OnDestroy {
  private logger: Logger = inject(LoggerFactory).createLogger(SettingsLandingComponent, NgxLoggerLevel.ERROR);
  ...
  async ngOnInit() {
    ...
    this.defaultPageContent = {
      path: PageContentPath.ADMIN_SETTINGS_ACTION_BUTTONS,
      rows: [{
        maxColumns: 3,
        showSwiper: false,
