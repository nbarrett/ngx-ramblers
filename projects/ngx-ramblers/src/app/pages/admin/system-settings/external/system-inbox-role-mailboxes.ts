  ...
  template: `
    <div class="row">
      <div class="col-sm-12 px-0">
        <p class="text-muted">
          Role mailboxes appear here when a committee role's Inbound Forwarding (in <a href="/admin/system-settings?tab=committee">Committee Settings</a>)
          points at one of the connected Gmail accounts. This view is read-only.
        </p>
        @if (aliases.length === 0) {
          <div class="alert alert-info py-2 small mb-0">
            No committee roles are mapped to a connected Gmail inbox yet. Open Committee Settings and set the Inbound Forwarding
            for each role to point at the Gmail account that should receive its mail.
          </div>
        } @else {
