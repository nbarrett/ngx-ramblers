...
export class MailProviderSettingsComponent implements OnInit, OnDestroy {
  ...
  public mailchimpListTypes: KeyValue<string>[] = [];
  ...
  async ngOnInit() {
    ...
    this.subscriptions.push(this.mailMessagingService.events().subscribe(async mailMessagingConfig => {
      ...
      this.mailchimpListTypes = this.mailchimpConfigService.configuredListTypes(this.mailchimpConfig);
  ...
  clearAllSubscriptions() {
    switch (this.config?.mailDefaults?.mailProvider) {
      ...
      case MailProvider.MAILCHIMP:
        const configuredKeys = this.mailchimpListTypes.filter(item => item.value).map(item => item.key);
  ...
  async calculateMailProviderStats() {
    ...
    switch (this.config?.mailDefaults?.mailProvider) {
      case MailProvider.BREVO:
        const mailProviderStats = this.mailListUpdaterService.mailProviderStats(this.groupMembers);
