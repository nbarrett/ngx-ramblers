                ...
                <div class="row thumbnail-heading-frame">
                  ...
                  <app-sortable-table
                    ...
                    <ng-template appSortableTableGroupHeader let-group>
                      {{ groupLabel(group) }} — {{ groupStart(group) | date:'EEE d MMM yyyy' }}
                      <span class="text-muted fw-normal ms-2">{{ groupSummary(group) }}</span>
...
export class BackupAndRestore implements OnInit, OnDestroy {
  ...
  refreshHistoryRows() {
    this.historyRows = this.filteredSessions().map(session => ({
      ...session,
  ...
  historyGroupBy = (row: BackupSessionHistoryRow): string => {
    return `${this.dateUtils.asDateTime(row.startTime).toISODate()}|${this.sessionTrigger(row)}`;
  };
  ...
  groupStart(group: SortableTableGroup<BackupSessionHistoryRow>): number {
    return Math.max(...group.rows.map(row => row.startedMs));
  }
  ...
  groupSummary(group: SortableTableGroup<BackupSessionHistoryRow>): string {
    const completed = group.rows.filter(row => row.status === BackupSessionStatus.COMPLETED).length;
    const failed = group.rows.filter(row => row.status === BackupSessionStatus.FAILED).length;
    const inProgress = group.rows.filter(row => row.status === BackupSessionStatus.IN_PROGRESS).length;
    const parts = [
      `${completed} completed`,
      failed > 0 ? `${failed} failed` : null,
      inProgress > 0 ? `${inProgress} in progress` : null
    ].filter(part => part !== null);
    return `${this.stringUtils.pluraliseWithCount(group.rows.length, "session")}: ${parts.join(", ")}`;
  }
