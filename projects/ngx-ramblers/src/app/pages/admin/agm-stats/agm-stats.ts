...
@Component({
  ...
  template: `
      ...
      @if (stats) {
        <tabset class="custom-tabset">
          ...
          <tab app-agm-expenses-tab
               ...
               [changeClassFn]="changeClassFn"
               [getYearLabelFn]="getYearLabelFn"
               [yearlyStats]="getYearlyStatsReversed()"
    ...
    <ng-template #dateRangeControls>
      @if (loading) {
        <div class="alert alert-warning alert-dismissible fade show loading-alert" role="alert">
...
export class AGMStatsComponent implements OnInit {
  private logger: Logger = inject(LoggerFactory).createLogger("AGMStatsComponent", NgxLoggerLevel.INFO);
  ...
  prepareChartData() {
    ...
    const formatDateRange = (fromTimestamp: number, toTimestamp: number): string => {
      const from = datePipe.transform(fromTimestamp, "dd MMM yyyy");
      const to = datePipe.transform(toTimestamp, "dd MMM yyyy");
      return `${from} - ${to}`;
    };
  ...
  expenseSummaryRows() {
    ...
    return [
      ...
      {metric: "Total Cost", values: periods.map(p => this.periodValue(p, "expenses", "totalCost"))}
    ].map(row => {
      const previous = row.values[row.values.length - 2] ?? 0;
      const current = row.values[row.values.length - 1] ?? 0;
      ...
      return {
        ...
        current,
        changeValue: current - previous,
        displayValues: row.metric === "Total Cost" ? row.values.map(toCurrency) : row.values,
        totalForPeriod: row.metric === "Total Cost" ? toCurrency(total) : total,
        changeDisplay: this.percentageChange(current, previous)
      };
    });
  }
