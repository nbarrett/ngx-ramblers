      ...
      @if (loggedIn) {
        @if (ngxLiteService.ngxLite) {
          <div class="alert alert-warning" role="alert">
            Content management is not available in NGX-Lite mode. Switch to full mode in Environment Management to manage pages, articles, media, walks, and events.
          </div>
        } @else {
...
export class ContentLandingComponent implements OnInit, OnDestroy {
  private logger: Logger = inject(LoggerFactory).createLogger(ContentLandingComponent, NgxLoggerLevel.ERROR);
  ...
  async ngOnInit() {
    ...
    this.defaultPageContent = {
      path: PageContentPath.ADMIN_CONTENT_ACTION_BUTTONS,
      rows: [{
        maxColumns: 3,
        showSwiper: false,
