...
export const MAIL_SETTINGS_UNSUBSCRIBES_HELP: ContentText = {
  name: "mail-settings-unsubscribes-help",
  category: "admin",
  text:
    "* Lists every contact Brevo is currently suppressing. Toggle between **Unsubscribes** and **Blocks** at the top right.\n" +
    "* Filter by reason, sender or date range. Click the trash icon to remove a contact from Brevo's blocklist.\n" +
    "* Synced every two hours via cron and in real time via Brevo webhooks.\n" +
...
export function findContentEntry(name: string, category: string): ContentText | undefined {
  return DEFAULT_CONTENT_ENTRIES.find(e => e.name === name && e.category === category);
}
