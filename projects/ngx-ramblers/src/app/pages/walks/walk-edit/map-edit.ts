...
export class MapEditComponent implements OnInit, OnDestroy {
  ...
  private logger: Logger = inject(LoggerFactory).createLogger("MapEditComponent", NgxLoggerLevel.INFO);
  private zone = inject(NgZone);

  async ngOnInit() {
    this.initializeSubscriptions();
    this.logger.debug("locationDetails:", this.locationDetails);
    await this.setDefaultLatLng();
    this.initializeMap();
  }
  ...
  private async setDefaultLatLng() {
  ...
  private initializeMap() {
    ...
    if (this.locationDetails.latitude !== undefined && this.locationDetails.longitude !== undefined) {
      this.configureMap();
    } else {
  ...
  private configureMap() {
    ...
    this.layers = [
      L.marker([latitude, longitude], {draggable: true}).on("dragend", (event) =>
  ...
  private async updateWalkLocation(latlng: LatLng) {
    ...
    this.addressQueryService.gridReferenceLookupFromLatLng(latlng)
      .then((response: GridReferenceLookupResponse) => {
        ...
        if (response?.gridReference && response?.postcode) {
          const gridReference = response.gridReference;
          const postcode = response.postcode;
          const description = response.description;
          this.logger.info("Location updated:", {postcode, gridReference, description});
          this.locationDetails.postcode = postcode;
          this.locationDetails.grid_reference_10 = gridReference;
