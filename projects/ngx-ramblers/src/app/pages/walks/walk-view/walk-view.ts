...
@Component({
          ...
          <div class="row">
            ...
            <div class="col-sm-12 col-lg-6 rounded">
              @if (displayedWalk?.walk?.groupEvent?.media?.length > 0) {
                <div class="row">
                  <div class="col-sm-12">
                    <app-walk-images [extendedGroupEvent]="displayedWalk.walk"/>
                  </div>
                </div>
              }
  ...
  imports: [WalkPanelExpanderComponent, TooltipDirective, MarkdownComponent, WalkLeaderComponent, WalkFeaturesComponent, FontAwesomeModule, RouterLink, WalkImagesComponent, MapEditComponent, FormsModule, NgClass, WalkDetailsComponent, DisplayDayPipe, RelatedLinksComponent, DisplayTimePipe]
})
...
export class WalkViewComponent implements OnInit, OnDestroy {
  ...
  queryIfRequired(): void {
    if (this.pathContainsWalkId) {
      this.walksAndEventsService.getByIdIfPossible(this.walkIdOrPath)
  ...
  private applyWalk(displayedWalk: DisplayedWalk) {
    if (displayedWalk) {
      ...
      this.displayLinks = this.displayedWalk.walk?.fields?.links?.length > 0;
