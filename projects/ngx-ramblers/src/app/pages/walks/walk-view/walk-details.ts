    ...
    template: `
      <div class="event-panel rounded">
        <h1>
          ...
          <fa-icon [icon]=""
                   class="fa-icon mr-2"/>
          {{ display.isWalk(displayedWalk.walk) ? displayedWalk.walk.groupEvent.shape : null }}
        ...
        <div class="row">
          ...
          @if (displayedWalk?.walk?.groupEvent?.start_location?.description) {
            <div
              class="col-sm-12 mt-1">{{ displayedWalk?.walk?.groupEvent?.start_location?.description }}
            </div>
          }
