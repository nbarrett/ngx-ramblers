...
export class WalkAddSlotsComponent implements OnInit {
  ...
  private logger: Logger = inject(LoggerFactory).createLogger("WalkAddSlotsComponent", NgxLoggerLevel.INFO);
  ...
  addWalkSlots() {
    ...
    this.walksAndEventsService.all({
      ...
      .then((walks: ExtendedGroupEvent[]) => {
        ...
        const existingDates: number[] = this.extendedGroupEventQueryService.activeWalks(walks).map(walk => this.dateUtils.asValueNoTime(walk.groupEvent.start_date_time));
