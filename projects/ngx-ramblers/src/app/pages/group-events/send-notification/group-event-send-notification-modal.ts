...
import {
  GroupEventNotificationDetails
} from "../../../notifications/group-events/templates/group-event-notification-details";
...
@Component({
    selector: "app-group-event-send-notification-modal",
    template: `
    @if (mailMessagingConfig) {
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Send <em>Event</em> Notification</h4>
              ...
              <div class="row">
                <div class="col-sm-12">
                  <div class="form-group">
                    ...
                    <textarea [(ngModel)]="groupEvent.groupEvent.title"
              ...
              @if (latestNotification?.content?.eventDetails) {
                <div class="row">
                  <div class="col-sm-12">
                    ...
                    @if (groupEvent.fields.attendees.length>0) {
                      <div class="row">
                        <div class="col-sm-12">
                          <div class="form-group">
                            <div class="form-check">
                              ...
                              <label
                                ...
                                <span
                                  style="font-weight: normal"> ({{ display.attendeeList(groupEvent, display.memberFilterSelections) }}
                                )</span>
                              </label>
                            </div>
                          </div>
                ...
                <div class="col-sm-12">
                  <div class="form-group">
                    <div class="form-check">
                      ...
                      <label class="form-check-label"
                        for="include-description">Include Event Description text:
                      </label>
                    </div>
                  </div>
                </div>
                    ...
                    <div class="form-group">
                      <div class="form-check">
                        ...
                        <label class="form-check-label"
                          ...
                          <span
                          style="font-weight: normal"> {{ display.attachmentTitle(groupEvent) }}</span>
                        </label>
                      </div>
                    </div>
                  </div>
                ...
                <div class="col-sm-12">
                  <div class="form-group">
                    ...
                    <select [(ngModel)]="latestNotification.content.signoffAs.value"
                      (ngModelChange)="groupEventSignoffChanged($event)"
                      id="signoff-as"
    ...
    imports: [TabsetComponent, TabDirective, NotificationConfigSelectorComponent, FormsModule, TooltipDirective, NgSelectComponent, NgOptgroupTemplateDirective, CommitteeNotificationRamblersMessageItemComponent, GroupEventNotificationDetails, FontAwesomeModule, CreateOrAmendSenderComponent, BrevoButtonComponent, NotificationDirective, TitleCasePipe, EmailPreviewComponent]
})
export class GroupEventSendNotificationModal implements OnInit, OnDestroy {

  private logger: Logger = inject(LoggerFactory).createLogger("GroupEventSendNotificationModal", NgxLoggerLevel.ERROR);
  ...
  initialiseNotification() {
    if (!this.groupEvent.fields.notifications) {
      this.groupEvent.fields.notifications = [];
      this.logger.info("initialiseNotification:notifications initialised");
    }
    if (this.groupEvent.fields.notifications.length === 0 || !last(this.groupEvent.fields.notifications)?.content) {
      ...
      this.groupEvent.fields.notifications.push(this.latestNotification);
    ...
    this.defaultNotificationContentField(["attendees"], {include: this.groupEvent.fields.attendees.length > 0});
    this.defaultNotificationContentField(["attachment"], {include: !!this.groupEvent.fields.attachment});
    this.defaultNotificationContentField(["replyTo"], {
      include: true,
      value: this.committeeMemberForTypeOrBuiltInRole(this.groupEvent?.fields?.contactDetails?.displayName ? "organiser" : BuiltInRole.SOCIAL_CO_ORDINATOR)?.type
    });
  ...
  initialiseRoles(members: Member[]) {
    const roles = this.display.committeeMembersPlusOrganiser(this.groupEvent, members);
  ...
  saveAndSendLater() {
    this.walksAndEventsService.update(this.groupEvent).then(() => this.bsModalRef.hide());
  }
  ...
  saveGroupEvent() {
    return this.walksAndEventsService.createOrUpdate(this.groupEvent);
  }
  ...
  async createThenEditOrSendEmailCampaign(bodyContent: string, campaignName: string, createAsDraft: boolean) {
    ...
    if (this.latestNotification.content?.listId > 0) {
      ...
      this.notify.error({
        title: "Send Group Event notification",
  ...
  runCampaignCreationAndSendWorkflow(createAsDraft?: boolean) {
    if (!this.notReady()) {
    this.notify.setBusy();
    const campaignName = this.groupEvent?.groupEvent?.title;
    this.logger.info("sendGroupEventNotification:notification->", this.latestNotification);
  ...
  groupEventSignoffChanged(type: any) {
    this.logger.info("groupEventSignoffChanged:type", type);
  }
  ...
  signoffAs(): CommitteeMember {
    const signoffAs = this.display?.committeeMembersPlusOrganiser(this.groupEvent, this.members)?.find(member => this.latestNotification?.content?.signoffAs?.value === member.type);
  ...
  replyToRole(): CommitteeMember {
    const membersPlusOrganiser = this.display?.committeeMembersPlusOrganiser(this.groupEvent, this.members);
