                      ...
                      <div class="row">
                        <div class="col-sm-12">
                          <div class="form-group">
                            ...
                            <input [disabled]="!display.allow.edits"
                                   [(ngModel)]="groupEvent.groupEvent.url" type="text"
                                   class="form-control input-sm"
                                   id="url"/>
                          </div>
                        </div>
                    ...
                    <div class="row">
                      <div class="col-sm-8">
                        <div class="form-group">
                          ...
                          <input [disabled]="!display.allow.edits"
                                 [(ngModel)]="groupEvent.groupEvent.location.description"
                      ...
                      <div class="col-sm-4">
                        <div class="form-group">
                          ...
                          <input [disabled]="!display.allow.edits"
                                 [(ngModel)]="groupEvent.groupEvent.location.postcode"
                    ...
                    <div class="row">
                      ...
                      @if (!display.allow.edits) {
                        <p class="col-sm-12 rounded">
                          {{ display.attendeeList(groupEvent, display.memberFilterSelections) }}</p>
                      }
                    </div>
                  </div>
                </tab>
...
export class GroupEventEdit implements OnInit, OnDestroy {
    ...
    if (this.urlService.pathContainsEventIdOrSlug()) {
      ...
      this.walksAndEventsService.queryById(groupEventId).then(async data => {
        ...
        if (this.groupEvent) {
          ...
          this.notify.hide();
          this.selectedMemberIds = this.groupEvent.fields.attendees.map(attendee => attendee.id);
        } else {
  ...
  onChange() {
    this.groupEvent.fields.attendees = this.selectedMemberIds.map(item => this.memberService.toIdentifiable(item));
    this.logger.debug("attendees: ", this.groupEvent.fields.attendees);
  ...
  attendeeCaption() {
    return this.groupEvent && this.groupEvent.fields.attendees.length + (this.groupEvent.fields.attendees.length === 1 ? " member is attending" : " members are attending"
    );
  }
