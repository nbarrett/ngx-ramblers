...
import { faArrowLeft, faCircleExclamation, faEnvelope, faFloppyDisk, faHammer, faMagnifyingGlass, faRotate } from "@fortawesome/free-solid-svg-icons";
...
import { REGISTRATION_STEPS, RamblersDirectoryLogo, RegistrationPage, RegistrationPageAnchor, RegistrationPlan, RegistrationState, RegistrationStep, SiteRegistration } from "../../models/site-registration.model";
...
import { startCase, values } from "es-toolkit/compat";
...
@Component({
  ...
  imports: [FormsModule, RouterLink, StepperModule, NgSelectComponent, FontAwesomeModule, PageComponent, GroupSelector, SiteMapViewComponent, RegistrationProgressLogComponent, SecretInputComponent, SuggestedTextChooserComponent],
  template: `
    <app-page pageTitle="Register your group">
      <p>Create a free review site from your group's own public pages, photos and current Walks Manager programme. Your existing website is not changed, and your group decides whether to continue after reviewing the result. <a routerLink="/how-to/group-registration" target="_blank" rel="noopener noreferrer">Learn how registration works</a>.</p>
      ...
      } @else if (message && activeStepKey() !== Step.CONTENT) {
          ...
          <p-step-item [value]="index + 1">
            ...
            <p-step-panel><ng-template pTemplate="content">
              @switch (step.key) {
                ...
                @case (Step.PROGRESS) {
                  <p>Status: <strong>{{readableLabel(registration?.state)}}</strong></p>
...
export class SiteRegistrationComponent implements OnInit, OnDestroy {
  ...
  readonly icons = {email: faEnvelope, search: faMagnifyingGlass, build: faHammer, refresh: faRotate, back: faArrowLeft, save: faFloppyDisk};
  ...
  readableLabel(value: string): string {
    return startCase(value || "Not started");
  }
