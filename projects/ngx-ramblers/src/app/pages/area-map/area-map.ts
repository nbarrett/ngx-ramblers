...
export class AreaMapComponent implements OnInit, OnDestroy {
  ...
  private initializeCmsMode() {
    ...
    this.cmsSettings = this.cmsService.initialize(this.row!, this.pageContent);
    ...
    if (!this.cmsSettingsSubscription) {
      this.cmsSettingsSubscription = this.cmsService.settingsChanges().subscribe(settings => {
        if (!settings || !this.row) {
          return;
        }
        const providerChanged = settings.provider && settings.provider !== this.provider;
        const styleChanged = settings.osStyle && settings.osStyle !== this.osStyle;
        const heightChanged = isNumber(settings.mapHeight) && settings.mapHeight !== this.mapHeight;
        const zoomChanged = isNumber(settings.mapZoom) && settings.mapZoom !== this.savedZoom;
        const centerChanged = settings.mapCenter && isArray(settings.mapCenter) &&
          (!this.savedCenter || settings.mapCenter[0] !== this.savedCenter.lat || settings.mapCenter[1] !== this.savedCenter.lng);
        const selectedGroupsChanged = settings.selectedGroups &&
          JSON.stringify(settings.selectedGroups) !== JSON.stringify(this.selectedGroups);

        this.region = settings.region || this.region;
        this.mapHeight = settings.mapHeight || this.mapHeight;
        this.provider = (settings.provider as MapProvider) || this.provider;
        this.osStyle = settings.osStyle || this.osStyle;
        this.opacityNormal = settings.opacityNormal ?? this.opacityNormal;
        this.opacityHover = settings.opacityHover ?? this.opacityHover;
        this.textOpacity = settings.textOpacity ?? this.textOpacity;
        this.areaColors = settings.areaColors || this.areaColors;
        this.selectedGroups = settings.selectedGroups || [];

        if (providerChanged || styleChanged) {
          this.recreateMap(true);
        } else if (heightChanged) {
          setTimeout(() => this.mapRef?.invalidateSize(true), 0);
        } else if (selectedGroupsChanged) {
          this.updateMap();
        }

        if (zoomChanged && this.mapRef && isNumber(settings.mapZoom)) {
          this.savedZoom = settings.mapZoom;
          this.mapRef.off("zoomend");
          this.mapRef.setZoom(settings.mapZoom);
          setTimeout(() => {
            if (this.mapRef) {
        ...
        if (centerChanged && this.mapRef && settings.mapCenter) {
          this.savedCenter = L.latLng(settings.mapCenter[0], settings.mapCenter[1]);
  ...
  onProviderChange(value: MapProvider) {
    if (this.standalone) {
      ...
      this.provider = value;
      if (this.cmsSettings) {
        this.cmsSettings.provider = value;
      }
      this.cmsService.updateSetting("provider", value);
      if (value === "os" && !this.osStyle) {
        this.osStyle = "Outdoor_27700";
        if (this.cmsSettings) {
          this.cmsSettings.osStyle = this.osStyle;
        }
        this.cmsService.updateSetting("osStyle", this.osStyle);
      }
      this.recreateMap(true);
    }
  }
  ...
  onStyleChange(value: string) {
    if (this.standalone) {
      ...
      this.osStyle = value;
      if (this.cmsSettings) {
        this.cmsSettings.osStyle = value;
      }
      this.cmsService.updateSetting("osStyle", value);
      this.recreateMap(true);
    }
  }
  ...
  onHeightChange(value: number) {
    if (this.standalone) {
      ...
      this.mapHeight = value;
      if (this.cmsSettings) {
        this.cmsSettings.mapHeight = value;
      }
      this.cmsService.updateSetting("mapHeight", value);
    }
    setTimeout(() => {
  ...
  onGroupSelectionChange() {
    if (this.row) {
      this.cmsService.updateSetting("selectedGroups", this.selectedGroups);
    }
    this.updateMap();
  }
  ...
  private rebuildMapWithGeoJSON() {
    ...
    if (this.preserveNextView && this.savedZoom) {
      ...
      if (parsedZoom >= 2 && parsedZoom <= 18) {
        ...
        if (this.row) {
          this.cmsService.updateSetting("mapZoom", 9);
        } else {
          ...
          polygon.on("mousemove", (e) => {
            if (!this.hoverTimeout && !popupShown) {
              this.hoverTimeout = setTimeout(() => {
                ...
                this.showLeafletPopup(e, area, polygon);
                popupShown = true;
              }, 800);
            }
          });
    ...
    if (this.mapRef && this.standalone) {
      ...
      if (currentZoom && isFinite(currentZoom) && currentZoom >= 2 && currentZoom <= 18) {
        ...
        if (this.row.areaMap) {
          ...
          this.cmsService.updateSetting("mapZoom", currentZoom);
        }
      }
    } else {
  ...
  private showLeafletPopup(e: L.LeafletMouseEvent, area: { name: string; url: string; groupCode?: string }, layer: L.Layer) {
    const content = `
      <div style="text-align: center;">
        ...
        ${area.url ? `<button type="button" class="badge bg-primary border-0 me-1" onclick="window.location.href='${area.url}'">visit group</button>` : ''}
        ${area.groupCode ? `<button type="button" class="badge bg-primary border-0" onclick="window.location.href='/walks?${StoredValue.SEARCH}=${encodeURIComponent(area.groupCode)}'">view walks</button>` : ''}
      </div>
    `;
  ...
  private handleMoveEnd() {
    if (this.mapRef && this.standalone) {
      ...
      if (this.row.areaMap) {
        ...
        this.cmsService.updateSetting("mapCenter", [center.lat, center.lng]);
      }
    } else {
